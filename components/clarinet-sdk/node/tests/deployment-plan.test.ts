...
// all manifests now boot with all Epoch40 boot contracts = 30
const bootContractsCount = 30;
...
describe("initSimnet deployment plans test", async () => {
  ...
  it("can use custom deployment plan", async () => {
    ...
    expect(contracts.size).toBe(bootContractsCount + 5);
  ...
  it("re-applies contract call tx when the deployment plan is updated", async () => {
    ...
    const contracts = simnet.getContractsInterfaces();
    expect(contracts.size).toBe(bootContractsCount + 2);
