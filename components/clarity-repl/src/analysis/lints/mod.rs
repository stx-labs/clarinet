mod at_block;
mod case_binding;
mod case_const;
mod case_data_var;
mod case_fn;
mod case_map;
mod case_token;
mod case_trait;
mod error_const;
mod flatten_variadic;
mod noop;
mod panic;
mod renamed_with_stacking;
mod unnecessary_as_max_len;
mod unnecessary_public;
mod unnecessary_tuple;
mod unused_binding;
mod unused_const;
mod unused_data_var;
mod unused_map;
mod unused_private_fn;
mod unused_token;
mod unused_trait;

pub use at_block::AtBlock;
pub use case_binding::CaseBinding;
pub use case_const::CaseConst;
pub use case_data_var::CaseDataVar;
pub use case_fn::CaseFn;
pub use case_map::CaseMap;
pub use case_token::CaseToken;
pub use case_trait::CaseTrait;
pub use error_const::ErrorConst;
pub use flatten_variadic::FlattenVariadic;
pub use noop::NoopChecker;
pub use panic::PanicChecker;
pub use renamed_with_stacking::{check_renamed_with_stacking, RenamedWithStacking};
pub use unnecessary_as_max_len::UnnecessaryAsMaxLen;
pub use unnecessary_public::UnnecessaryPublic;
pub use unnecessary_tuple::UnnecessaryTuple;
pub use unused_binding::UnusedBinding;
pub use unused_const::UnusedConst;
pub use unused_data_var::UnusedDataVar;
pub use unused_map::UnusedMap;
pub use unused_private_fn::UnusedPrivateFn;
pub use unused_token::UnusedToken;
pub use unused_trait::UnusedTrait;

#[cfg(test)]
mod tests {
    use clarity::vm::diagnostic::Level;
    use indoc::indoc;

    use super::{CaseConst, UnusedConst};
    use crate::analysis::linter::Lint;
    use crate::repl::session::Session;
    use crate::repl::SessionSettings;

    #[test]
    fn allow_with_annotation_at_error_level() {
        #[rustfmt::skip]
        let snippet = indoc!("
            ;; #[allow(unused_const, case_const)]
            (define-constant this_is_regular_snake_case u100)
        ").to_string();

        let mut settings = SessionSettings::default();
        settings
            .repl_settings
            .analysis
            .enable_lint(UnusedConst::get_name(), Level::Error);
        settings
            .repl_settings
            .analysis
            .enable_lint(CaseConst::get_name(), Level::Error);

        let (_, result) = Session::new_without_boot_contracts(settings)
            .formatted_interpretation(snippet, Some("checker".to_string()), false, None)
            .expect("Invalid code snippet");

        assert_eq!(result.lint_diagnostics.len(), 0);
    }
}
