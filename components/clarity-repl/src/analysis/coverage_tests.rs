...
#[test]
fn unwrap_ret_branching() {
    let contract = [
        "(define-public (must-unwrap (opt (optional uint)))",
        "  (ok (unwrap! opt (err u1)))",
        ")",
    ]
    .join("\n");

    // NOTE: unwrap! incorrectly always evaluates both branches. This will
    // need to change if it's fixed upstream
    let snippets: Vec<String> = vec!["(contract-call? .contract-0 must-unwrap (some u42))".into()];
    ...
    // calling multiple times scales the hit counts
    let snippets: Vec<String> = vec![
        "(contract-call? .contract-0 must-unwrap (some u1))".into(),
        "(contract-call? .contract-0 must-unwrap (some u2))".into(),
        "(contract-call? .contract-0 must-unwrap none)".into(),
    ];
    let cov = get_coverage_report(&contract, snippets);
    let expect = get_expected_report(
        [
...
#[test]
fn unwrap_err_ret_branching() {
    let contract = [
        "(define-public (must-fail (res (response uint uint)))",
        "  (ok (unwrap-err! res (err u1)))",
        ")",
    ]
    .join("\n");

    // NOTE: unwrap-err! incorrectly always evaluates both branches. This will
    // need to change if it's fixed upstream
    let snippets = vec!["(contract-call? .contract-0 must-fail (err u42))".into()];
    let cov = get_coverage_report(&contract, snippets);
    let expect = get_expected_report(
        [
            "FN:1,must-fail",
