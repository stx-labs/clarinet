...
/// Shared brand symbol file. Declared once per typegen run and imported by every
/// per-contract file so that the phantom `[__contractBrand]` property on bundled
/// contract consts resolves to the same `unique symbol` across the project.
pub const BRAND_TS: &str = "// This file is auto-generated by clarinet typegen\n\
...
pub fn generate_contract_types(contract_name: &str, interface: &ContractInterface) -> String {
    ...
    // --- Bundled contract value (runtime ABI + phantom typed handle) ---
    //
    // Emit the runtime ABI as a literal `as const` value, then re-type it with a
    // phantom `[__contractBrand]: <Pascal>Contract` property. Consumers can read
    // the brand to infer the named contract interface without an explicit generic.
    let camel_name = clarity_to_camel(contract_name);
    let bundle_const = format!("{camel_name}Contract");
    let base_const = format!("_{bundle_const}Base");
    // `serde_json` is in dependencies; the only failure mode would be a non-string
    // map key, which `ContractInterface` does not produce. Unwrap is appropriate.
    let abi_json = serde_json::to_string_pretty(interface)
        .expect("ContractInterface always serializes");

    writeln!(body).unwrap();
    writeln!(body, "// Bundled contract: runtime ABI fused with typed interface").unwrap();
    writeln!(body, "const {base_const} = {abi_json} as const;").unwrap();
    writeln!(
        body,
        "export const {bundle_const} = {base_const} as typeof {base_const} & {{ readonly [__contractBrand]: {pascal_name}Contract }};"
    )
    .unwrap();
    ...
    // Brand symbol is a value-position reference (used as a computed key on a
    // type literal), so it must be a regular import rather than `import type`.
    writeln!(output, "import {{ __contractBrand }} from \"./brand\";\n").unwrap();
...
pub fn generate_index(contract_names: &[&str]) -> String {
    ...
    writeln!(output, "export * from \"./brand\";").unwrap();
...
mod tests {
    ...
    fn test_generate_contract_types_basic() {
        ...
        // Private function must not appear as a typed function alias or in the
        // CounterContract interface — only the bundled ABI value mirrors the
        // full on-chain interface and may legitimately include it.
        assert!(!output.contains("HelperArgs"));
        assert!(!output.contains("HelperReturn"));
    ...
    fn test_bundled_contract_const() {
        ...
        let output = generate_contract_types("simple-nft", &interface);

        // Brand symbol is imported as a value (used as computed key in a type position)
        assert!(output.contains("import { __contractBrand } from \"./brand\";"));
        // Base const carries the as-const runtime ABI
        assert!(output.contains("const _simpleNftContractBase = {"));
        assert!(output.contains("} as const;"));
        // The runtime payload includes the actual ABI fields
        assert!(output.contains("\"functions\""));
        assert!(output.contains("\"increment\""));
        // Exported bundle re-types the base const with the phantom brand
        assert!(output.contains(
            "export const simpleNftContract = _simpleNftContractBase as typeof _simpleNftContractBase & { readonly [__contractBrand]: SimpleNftContract };"
        ));
    }

    #[test]
    fn test_brand_ts_content() {
        // Sanity check: the shared brand file declares the unique symbol once.
        assert!(BRAND_TS.contains("export declare const __contractBrand: unique symbol;"));
    }
