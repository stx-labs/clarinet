...
pub fn main() {
    ...
    match opts.command {
        ...
        Command::Typegen(cmd) => {
            ...
            // Approach A: concrete TypeScript types (generated by Rust)
            match clarity_ts_typegen::generate(&contracts, &output_dir.join("typed")) {
                Ok(count) => {
                    println!(
                        "{} Generated concrete types for {} in {output_dir_name}/typed",
                        green!("✔"),
                        pluralize!(count, "contract"),
                    );
                }
                Err(e) => {
                    eprintln!("{} typed: {e}", red!("error:"));
                    process::exit(1);
                }
            }

            // Approach B: ABI as const + TypeScript utility types
            match clarity_abi_typegen::generate(&contracts, &output_dir.join("abi")) {
                Ok(count) => {
                    println!(
                        "{} Generated ABI consts for {} in {output_dir_name}/abi",
                        green!("✔"),
                        pluralize!(count, "contract"),
                    );
                }
                Err(e) => {
                    eprintln!("{} abi: {e}", red!("error:"));
                    process::exit(1);
                }
            }
        }
