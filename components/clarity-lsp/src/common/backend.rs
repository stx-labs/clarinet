...
"#;
        ...
        assert_eq!(
            diagnostics_array.len(),
            1,
            "Spans array should have one item"
        );
        let span = spans_array.get(0).expect("Failed to get expected span");
        let start_line = span
            .get("start_line")
        ...
        assert_eq!(
            start_line,
            &Value::Number(Number::from_string_unchecked("9".to_string()))
        );
        assert_eq!(
            end_line,
            &Value::Number(Number::from_string_unchecked("11".to_string()))
        );
    }
