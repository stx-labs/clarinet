...
use super::cost_analysis::{is_function_definition, CostAnalysisNode, CostExprNode};
...
impl Default for TraitCountCollector {
    fn default() -> Self {
        Self::new()
    }
}
        ...
        match native_function {
            ...
            NativeFunctions::TupleGet => {
                ...
                if node.children.len() >= 2 {
                    if let Some(field_node) = node.children.first() {
            ...
            NativeFunctions::ContractCall => {
                ...
                if let Some(first_arg) = node.children.first() {
        ...
        match native_function {
            NativeFunctions::Map | NativeFunctions::Filter | NativeFunctions::Fold => {
                if node.children.len() > 1 {
                    ...
                    if let Some(function_node) = node.children.first() {
            ...
            NativeFunctions::Let => {
                ...
                if let Some(bindings_tree) = node.children.first() {
