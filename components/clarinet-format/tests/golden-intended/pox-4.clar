...
(define-read-only (get-stacker-info (stacker principal))
    (match (map-get? stacking-state { stacker: stacker })
        stacking-info
        (if (<=
                (+ (get first-reward-cycle stacking-info)
                    (get lock-period stacking-info)
                )
            ...
            (reward-set-index (if (< i num-cycles)
                (let (
                        ...
                        (reward-index
                            ...
                            (append-reward-cycle-pox-addr (get pox-addr params)
                                reward-cycle (get amount-ustx params)
                                (get stacker params) (get signer params)
                            ))
                    )
...
(define-private (add-pox-addr-to-reward-cycles
    ...
    (let (
        ...
        (asserts! (is-eq num-cycles (get i results))
...
(define-public (stack-stx
    ...
    (let (
        ...
        (try! (consume-signer-key-authorization pox-addr (- first-reward-cycle u1)
            "stack-stx" lock-period signer-sig signer-key amount-ustx
            max-amount auth-id
        ))
        ...
        (match signer-sig-opt
            ...
            (ok (asserts!
                (is-eq
                    (unwrap!
                        (secp256k1-recover?
                            (get-signer-key-message-hash pox-addr reward-cycle
                                topic period max-amount auth-id
                            )
                            signer-sig
                        )
...
(define-private (inner-stack-aggregation-commit
    ...
    (let ((partial-stacked
        ...
        (let ((amount-ustx (get stacked-amount partial-stacked)))
            (try! (consume-signer-key-authorization pox-addr reward-cycle "agg-commit"
                u1 signer-sig signer-key amount-ustx max-amount auth-id
            ))
...
(define-public (stack-aggregation-increase
    ...
    (let ((partial-stacked
        ...
        (let (
            ...
            (try! (consume-signer-key-authorization pox-addr reward-cycle
                "agg-increase" u1 signer-sig signer-key
                increased-entry-total max-amount auth-id
            ))
...
(define-public (delegate-stack-stx
    ...
    (let (
        ...
        ;; ensure that stacking can be performed
        (try! (minimal-can-stack-stx pox-addr amount-ustx first-reward-cycle
            lock-period
        ))
        ...
        (add-pox-partial-stacked pox-addr first-reward-cycle lock-period
            amount-ustx
        )
    ...
    (let (
        ...
        (if (> first-cycle reward-cycle)
            ...
            (let (
                ...
                ;; stacker must match
                (asserts!
                    (is-eq (get stacker existing-entry) (some (get stacker data)))
                    none
                )
...
(define-public (stack-increase
    ...
    (let (
        ...
        (asserts!
            (is-some (fold increase-reward-cycle-entry
...
(define-public (stack-extend
    ...
    (let (
            ...
            (stacker-state (unwrap! (get-stacker-info tx-sender)
        ...
        (let (
            ...
            ;;   for the new cycles
            (let (
                    (extended-reward-set-indexes (try! (add-pox-addr-to-reward-cycles pox-addr first-extend-cycle
                        extend-count amount-ustx tx-sender signer-key
                    )))
                    (reward-set-indexes
                        ;; use the active stacker state and extend the existing reward-set-indexes
                        (let (
                                (cur-cycle-index (- first-reward-cycle
                                    (get first-reward-cycle stacker-state)
                                ))
                                ...
                                (new-list (concat
                                    (default-to (list)
                                        (slice? old-indexes cur-cycle-index
                                            (len old-indexes)
                                        ))
...
(define-public (delegate-stack-extend
    ...
    (let (
        ...
        (let (
            ...
            ;; delegate stacking does minimal-can-stack-stx
            (try! (minimal-can-stack-stx pox-addr amount-ustx first-extend-cycle
                lock-period
            ))
